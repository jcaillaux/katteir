...
#
# Usage: tools/encode.sh in.webm out.ivf [fps=30] [height=720] [crf=30]
#
# A keyframe only every 10 s: clips are only ever played from the start, and
# each keyframe costs (every 2 s made the ginger cat's loop 40 % bigger at
# the same quality).
set -eu
...
ffmpeg -v error -y $DECODER -i "$IN" -an \
    ...
    -c:v libsvtav1 -preset 8 -crf "$CRF" -g $((FPS * 10)) \
    -f ivf "$OUT"
