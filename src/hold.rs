...
//! nothing here reads the clock. Used by the cat window (`overlay.rs`); the
//! hold duration comes from `display.dismiss_hold_secs`.
...
mod tests {
    ...
    fn short_press_does_not_dismiss_and_resets() {
        ...
        hold.release();
        assert!(hold.progress(t0 + secs(4.9)).abs() < f32::EPSILON);
    ...
    fn full_hold_dismisses_exactly_once() {
        ...
        assert!((hold.progress(t0 + secs(6.0)) - 1.0).abs() < f32::EPSILON);
    }
    ...
    fn clock_going_backwards_is_harmless() {
        ...
        let mut hold = HoldToDismiss::new(HOLD);
        hold.press(t0);
        let before = t0.checked_sub(secs(1.0)).expect("t0 is 10 s in the future");
        assert!(hold.progress(before).abs() < f32::EPSILON);
        assert!(!hold.tick(before));
    }
