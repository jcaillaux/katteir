...
//! That global also says whether the `clip-fields` feature is on.
...
fn main() {
    ...
    // Cargo sets this for build scripts when the feature is on.
    let clip_fields = std::env::var_os("CARGO_FEATURE_CLIP_FIELDS").is_some();
    let app_info = out_dir.join("app_info.slint");
    let slint = format!(
        "// Generated by build.rs from Cargo.toml.\nexport global AppInfo {{\n    out property <string> name: \"{name}\";\n    out property <bool> clip-fields: {clip_fields};\n}}\n"
    );
